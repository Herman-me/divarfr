	...
	class Database
	{
		...
		*/
		...
		public function record()
		 ...
		 // update_settings
		public function update_tite($val)
		{
			$val = $this->escape($val);
			$update = $this->link->query("UPDATE info SET web_name='$val' WHERE id=1");
			return $update;
		}
		...
		**/
		...
		//Method for update tel infos inside database
		public function tel_info_update($api,$chat_id)
		{
			// Lets Escape theme
			$array = array($api,$chat_id);
			$es    = $this->escape($array);
			$api   = $es[0];
			$chai  = $es[1];

			$chais = preg_replace("/[@]/", '', $chai);
			// Update the informations
			$update = $this->link->query("UPDATE telegram SET api='$api' , chat_id='$chais'");

			$this->close(); // Close database connections
